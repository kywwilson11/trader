...
TRAIN_RATIO = 0.8
NUM_CLASSES = 3
# Prune trials that are clearly bad after this many epochs
...
def get_indices_and_classes(bull_thresh, seq_len):
...
def objective(trial):
    ...
    # --- Continuous/intelligent sampling via Optuna TPE ---
    ...
    valid_indices, classes = get_indices_and_classes(bull_threshold, seq_len)
    ...
    # Store extra info for later retrieval
...
# --- Callback: print progress + save log ---
results_log = []
t0 = time.time()
...
def trial_callback(study, trial):
    ...
    results_log.append({
        ...
        'trading_score': score, 'per_class': pc,
...
# --- MAIN SEARCH ---
print(f"\n{'='*70}")
print(f"OPTUNA HYPERPARAMETER SEARCH: {NUM_TRIALS} trials (TPE + pruning)")
print(f"{'='*70}\n")
...
study.optimize(objective, n_trials=NUM_TRIALS, callbacks=[trial_callback])

# --- RESULTS ---
total_time = time.time() - t0
print(f"\n{'='*70}")
print(f"DONE: {NUM_TRIALS} trials in {total_time/60:.1f}min")
print(f"{'='*70}")

# Find best completed trial with val_acc > 0.34
best_trial = None
...
if best_trial is not None:
    ...
    print(f"\nBest (ts={best_trial.value:.3f}, acc={best_trial.user_attrs['val_acc']:.3f}):")
    ...
    torch.save(mdl.state_dict(), 'stock_predictor.pth')
    ...
    print("\nBest model saved. Ready for crypto_loop.py")

    # Param importance (if enough completed trials)
...
print(f"Log: hypersearch_log.json")
